...
main = do
    ...
    lightingShader <- simpleShaderProgram ("data" </> "2_Lighting" </> "5_Light-casters" </> "spotlight-soft.vs")
        ("data" </> "2_Lighting" </> "5_Light-casters" </> "spotlight-soft.frag")
...
drawScene lightingShader contVAO lampShader lightVAO w cam = do
    ...
    setUniform lightingShader "light.position" (position cam)
    setUniform lightingShader "light.direction" (front cam)
    setUniform lightingShader "light.cutOff" (cos (radians (12.5 :: GLfloat)))
    setUniform lightingShader "light.outerCutOff" (cos (radians (17.5 :: GLfloat)))
    setUniform lightingShader "viewPos" (position cam)

    setUniform lightingShader "light.ambient" (V3 (0.1 :: GLfloat) 0.1 0.1)
    setUniform lightingShader "light.diffuse" (V3 (0.8 :: GLfloat) 0.8 0.8)
